...
public class ETSITS102941MessagesCaGenerator {
    ...
    /**
     * Method to generate a initial self signed EnrolmentRequestMessage, for rekeying see separate method according to section 6.2.1 in ETSI TS 102 941 v 1.3.1
     * <p>
     *     <u>
     ...
     */
    public EncryptResult genInitialEnrolmentRequestMessage(Time64 generationTime, InnerEcRequest innerEcRequest, PublicKey signerPublicKey, PrivateKey signerPrivateKey, Certificate recipient) throws IllegalArgumentException, IOException, GeneralSecurityException {
     ...
     * @throws GeneralSecurityException if problems occurred encrypting the message.
     */
    public EncryptResult genRekeyEnrolmentRequestMessage(Time64 generationTime, InnerEcRequest innerEcRequest, EtsiTs103097Certificate[] oldCertificateChain, PrivateKey oldSignerPrivateKey, PublicKey signerPublicKey, PrivateKey signerPrivateKey, Certificate recipient) throws IOException, GeneralSecurityException {
        HeaderInfo headerInfo = genHeaderInfo(generationTime);
        EtsiTs103097DataSigned innerSigned = securedDataGenerator.genEtsiTs103097DataSigned(headerInfo, innerEcRequest.getEncoded(),signerPublicKey,signerPrivateKey);
        InnerEcRequestSignedForPop innerEcRequestSignedForPop = new InnerEcRequestSignedForPop(innerSigned.getProtocolVersion(),innerSigned.getContent());
        EtsiTs102941Data etsiTs102941Data = new EtsiTs102941Data(Version.V1,new EtsiTs102941DataContent(innerEcRequestSignedForPop));

        return securedDataGenerator.signAndEncryptData(headerInfo,etsiTs102941Data.getEncoded(), SecuredDataGenerator.SignerIdentifierType.HASH_ONLY,oldCertificateChain,oldSignerPrivateKey,getRecipientAlgorithm(recipient),new Recipient[]{new CertificateRecipient(recipient)});
    }
    ...
    public ECRequestVerifyResult<InnerEcRequest> decryptAndVerifyEnrolmentRequestMessage(EtsiTs103097DataEncryptedUnicast enrolmentRequestMessage,Map<HashedId8, Certificate> certStore, Map<HashedId8, Certificate> trustStore, Map<HashedId8, Receiver> receiverStore)
            throws MessageParsingException, SignatureVerificationException, DecryptionFailedException, InternalErrorException {
    ...
    /**
     * Method to generate a EnrolmentResponseMessage according to ETSI TS 102 941 v 1.3.1.
     * <p>
     ...
     *     which contains the HashedId8 of the SymmetricEncryptionKey structure containing the symmetric key used by the
     *     ITS-S to encrypt the <i>EnrolmentRequest</i> message to which the response is built;</li>
     ...
     *     <ul>
     *         <li>the <i>requestHash</i> is the left-most 16 octets of the SHA256 digest of the topmost
     *         EtsiTs103097Data - Encrypted structure received in the request;</li>
    ...
    public VerifyResult<InnerEcResponse> decryptAndVerifyEnrolmentResponseMessage(EtsiTs103097DataEncryptedUnicast enrolmentResponseMessage,
        ...
        byte[] decryptedData;
        try{
            decryptedData = securedDataGenerator.decryptData(enrolmentResponseMessage,receiverStore);
        }catch (Exception e){
            throw new DecryptionFailedException("Error, couldn't decrypt EnrolmentResponseMessage(: " + e.getMessage(),e);
        }
        try {
     ...
     */
        public EncryptResult genAuthorizationRequestMessage(Time64 generationTime, PublicKeys publicKeys, byte[] hmacKey, SharedAtRequest sharedAtRequest,
        ...
        if(withPrivacy){
            EncryptResult encryptedEcSignatureResult = securedDataGenerator.encryptData(getRecipientAlgorithm(enrolmentAuthorityRecipient),
                    ecSignaturePayload.getEncoded(),
                    new Recipient[] {new CertificateRecipient(enrolmentAuthorityRecipient)});
            ecSignature = new EcSignature((EtsiTs103097DataEncrypted) encryptedEcSignatureResult.getEncryptedData());
        }else{
        ...
        return securedDataGenerator.encryptData(getRecipientAlgorithm(authorizationAuthorityRecipient),
                data,
    ...
    public VerifyResult<EcSignature> decryptAndVerifyECSignature(EcSignature ecSignature, SharedAtRequest sharedAtRequest, boolean expectPrivacy,
        ...
        try {
            if (expectPrivacy && ecSignature.getType() == EcSignature.EcSignatureChoices.ecSignature) {
                throw new MessageParsingException("Invalid InnerATRequest received, ECSignature should be encrypted.", null);
            }
            ...
            if (!securedDataGenerator.verifyReferencedSignedData(ecSignaturePayload, sharedAtRequest.getEncoded(), enrolmentCACertStore, enrolmentCATrustStore)) {
                throw new SignatureVerificationException("Invalid external payload signature in ec signature of innerAtRequest.", null);
            }
            if (ecSignaturePayload.getContent().getType() != Ieee1609Dot2Content.Ieee1609Dot2ContentChoices.signedData) {
                throw new MessageParsingException("Invalid encoding in innerAtRequest, message type must be signed data", null);
            }
            ...
            if (signedData.getTbsData().getPayload().getExtDataHash() == null) {
                throw new MessageParsingException("Invalid encoding in innerAtRequest SignedData, extDataHash field cannot be null", null);
            }
    ...
    public VerifyResult<InnerAtResponse> decryptAndVerifyAuthorizationResponseMessage(EtsiTs103097DataEncryptedUnicast authorizationResponseMessage,
        ...
        byte[] decryptedData;
        try{
            decryptedData = securedDataGenerator.decryptData(authorizationResponseMessage,receiverStore);
        }catch (Exception e){
            throw new DecryptionFailedException("Error, couldn't decrypt AuthorizationResponseMessage: " + e.getMessage(),e);
        }
        try {
            EtsiTs103097DataSigned outerSignature = new EtsiTs103097DataSigned(decryptedData);
    ...
    /**
     * Method to generate a AuthorizationValidationRequestMessage according to ETSI TS 102 941 v 1.3.1.
     * <p>
     ...
     * @throws GeneralSecurityException if problems occurred encrypting the message.
     */
    public EncryptResult genAuthorizationValidationRequest(Time64 generationTime, AuthorizationValidationRequest authorizationValidationRequest,
        ...
        return securedDataGenerator.signAndEncryptData(headerInfo,etsiTs102941Data.getEncoded(),
    ...
    /**
     * Method to generate a AuthorizationValidationResponseMessage according to ETSI TS 102 941 v 1.3.1.
     * <p>
     ...
     *             <ul>
     *                 <li>the requestHash is the left-most 16 octets of the SHA256 digest of the topmost
     *                 EtsiTs103097Data-Encrypted structure received in the <i>AuthorizationValidationRequestMessage</i>
     *                 </li>
     ...
     *             of choice <i>pskRecipInfo</i>,which contains the HashedId8 of the SymmetricEncryptionKey structure
     *             containing the symmetric key used by the ITS-S to encrypt the <i>AuthorizationRequest</i> message to
     *             which the response is built;</li>
     *             <li>
     *               the component <i>cipherText</i> containing the encrypted representation of the
    ...
    public VerifyResult<AuthorizationValidationResponse> decryptAndVerifyAuthorizationValidationResponseMessage(EtsiTs103097DataEncryptedUnicast authorizationValidationResponseMessage,
        ...
        byte[] decryptedData;
        try{
            decryptedData = securedDataGenerator.decryptData(authorizationValidationResponseMessage,receiverStore);
        }catch (Exception e){
            throw new DecryptionFailedException("Error, couldn't decrypt AuthorizationValidationResponseMessage: " + e.getMessage(),e);
        }
        try {
            EtsiTs103097DataSigned outerSignature = new EtsiTs103097DataSigned(decryptedData);
    ...
    /**
     * Method to generate a CertificateRevocationListMessage according to ETSI TS 102 941 v 1.3.1.
     *
     ...
     * @throws MessageParsingException if problems occurred parsing the message.
     * @throws SignatureVerificationException if problems occurred verifying the signature of the message.
     * @throws DecryptionFailedException if problems occurred decrypting the message. Only applicable for encrypted messages.
     * @throws InternalErrorException if internal problems occurred when processing the message.
     */
    ...
    /**
     * Method to generate a TlmCertificateTrustListMessage according to ETSI TS 102 941 v 1.3.1.
     *
     * @param generationTime the time of the generation
    ...
    public VerifyResult<ToBeSignedTlmCtl> verifyTlmCertificateTrustListMessage(EtsiTs103097DataSigned tlmCertificateTrustListMessage,
                                                                               Map<HashedId8, Certificate> certStore,
                                                                               Map<HashedId8, Certificate> trustStore)
            throws MessageParsingException, InternalErrorException, SignatureVerificationException, DecryptionFailedException {
        try{
    ...
    /**
     * Method to generate a RcaCertificateTrustListMessage according to ETSI TS 102 941 v 1.3.1.
     *
     * @param generationTime the time of the generation
    ...
    /**
     * Method to generate a CaCertificateRequestMessage according to section 6.2.1 in ETSI TS 102 941 v 1.3.1
     * <p>
     ...
     * @throws InternalErrorException if internal problems occurred when processing the message.
     */
    public VerifyResult<CaCertificateRequest> verifyCACertificateRequestMessage(EtsiTs103097DataSigned caCertificateRequestMessage)
            throws  MessageParsingException, InternalErrorException, SignatureVerificationException, DecryptionFailedException {
        try{
    ...
    public VerifyResult<CaCertificateRequest> verifyCACertificateRekeyingMessage(EtsiTs103097DataSigned caCertificateRekeyingMessage,
            ...
            Certificate> trustStore) throws MessageParsingException, SignatureVerificationException, DecryptionFailedException, InternalErrorException {
        try {
    ...
    /**
     * Help method to convert internal exceptions to one of CA Message exception with access to the
